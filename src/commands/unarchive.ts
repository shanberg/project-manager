...
export async function unarchiveProject(nameOrPrefix: string): Promise<void> {
  ...
  if (!matched) {
    ...
    if (prefixMatches.length > 1) {
      ...
      console.error("No archived project found matching:", nameOrPrefix);
      if (folders.length > 0) {
        console.error("Archived projects:", folders.join(", "));
      }
    }
    process.exit(1);
  }

  const src = path.join(archivePath, matched);
  const dest = path.join(activePath, matched);

  await rename(src, dest);
  console.log("Unarchived:", matched);
}
