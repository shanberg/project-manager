...
export async function fetchNotes(
  ...
  try {
    const { stdout } = await runPmWithPrefs(prefs, ["notes", "show", projectName]);
...
export async function getNotes(prefs: PreferenceValues, projectName: string): Promise<NotesShowOutput> {
  const { stdout, stderr } = await runPmWithPrefs(prefs, ["notes", "show", projectName]);
...
export async function resolveNotesPath(
  ...
  try {
    const { stdout } = await runPmWithPrefs(prefs, ["notes", "path", projectName]);
    const p = stdout.trim();
    return p || null;
  } catch {
    return null;
...
/** Toggle one todo in notes (flip [ ] <-> [x]) and write back. */
export async function toggleTodoInNotes(
  ...
  const updated = replaceTodoRawLineInNotes(notes, todo.rawLine, newLine);
...
export async function addTodoToTodaySession(
  ...
  const updatedSessions = sessions.map((s, i) => (i === idx ? { ...s, body: newBody } : s));
...
export async function addTodoBeforeInNotes(
  ...
  const updated = replaceTodoRawLineInNotes(
    notes,
    beforeTodo.rawLine,
    `${newLine}\n${beforeTodo.rawLine}`
  );
...
export function updateNotesSection(
  notes: ProjectNotes,
  updates: Partial<Pick<ProjectNotes, "summary" | "problem" | "goals" | "approach" | "links" | "learnings">>
): ProjectNotes {
